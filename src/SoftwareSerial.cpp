...
bool SoftwareSerial::isValidGPIOpin(int8_t pin)
{
    ...
    // bootup and other critical operations Remmove the flash memory pins on
...
#    ifdef CONFIG_IDF_TARGET_ESP32
    ...
    return (pin == 1) || (pin >= 3 && pin <= 5) || (pin >= 12 && pin <= 15) ||
        (!psramFound() && pin >= 16 && pin <= 17) || (pin >= 18 && pin <= 19) ||
...
void SoftwareSerial::setRxGPIOPullUp()
{
    if (m_rxValid) {
        pinMode(
            m_rxPin,
            hasRxGPIOPullUp(m_rxPin) && m_rxGPIOPullupEnabled ? INPUT_PULLUP :
                                                                INPUT);
    }
}
    ...
    int isrBufCapacity)
{
    ...
    if (isValidRxGPIOpin(m_rxPin)) {
        ...
        m_buffer.reset(
            new circular_queue<uint8_t>((bufCapacity > 0) ? bufCapacity : 64));
        if (m_parityMode) {
            m_parityBuffer.reset(
                new circular_queue<uint8_t>((m_buffer->capacity() + 7) / 8));
            m_parityInPos = m_parityOutPos = 1;
        }
        m_isrBuffer.reset(new circular_queue<uint32_t, SoftwareSerial *>(
            (isrBufCapacity > 0) ? isrBufCapacity :
                                   m_buffer->capacity() *
                    (2 + m_dataBits + static_cast<bool>(m_parityMode))));
    ...
    if (m_rxValid && on != m_rxEnabled) {
        if (on) {
            ...
            if (m_bitCycles >= (ESP.getCpuFreqMHz() * 1000000UL) / 74880UL)
                attachInterruptArg(
                    digitalPinToInterrupt(m_rxPin),
                    reinterpret_cast<void (*)(void *)>(rxBitISR),
                    this,
                    CHANGE);
            else
                attachInterruptArg(
                    digitalPinToInterrupt(m_rxPin),
                    reinterpret_cast<void (*)(void *)>(rxBitSyncISR),
                    this,
                    m_invert ? RISING : FALLING);
        } else {
...
void SoftwareSerial::lazyDelay()
{
    ...
    // const int32_t ms        = remaining > 0 ?
    //            remaining / 1000L / static_cast<int32_t>(ESP.getCpuFreqMHz())
    //            : 0;
    // if (ms > 0) {
    //     delay(ms);
    // } else {
...
size_t IRAM_ATTR SoftwareSerial::write(
    const uint8_t *buffer,
    size_t size,
    SoftwareSerialParity parity)
{
    if (m_rxValid) {
        rxBits();
    }
...
void SoftwareSerial::rxBits()
{
    ...
    // Check that there was no new ISR data received in the meantime, inserting
    // an extraneous stop level bit out of sequence breaks rx.
    if (m_rxLastBit < m_pduBits - 1) {
        const uint32_t detectionCycles =
            (m_pduBits - 1 - m_rxLastBit) * m_bitCycles;
        if (!m_isrBuffer->available() &&
            ESP.getCycleCount() - m_isrLastCycle > detectionCycles) {
...
void SoftwareSerial::rxBits(const uint32_t isrCycle)
{
    ...
    while (bits > 0) {
        ...
        if (m_rxLastBit < (m_dataBits - 1)) {
            uint8_t dataBits =
                min(bits, static_cast<uint32_t>(m_dataBits - 1 - m_rxLastBit));
...
void IRAM_ATTR SoftwareSerial::rxBitISR(SoftwareSerial *self)
{
    ...
    // cycle's LSB is repurposed for the level bit
    if (!self->m_isrBuffer->push((curCycle | 1U) ^ !level))
